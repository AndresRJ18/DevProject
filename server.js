...
// =====================================================
// INTEGRACIÓN CON GITHUB (Auto-generación de repositorios)
// =====================================================
app.post('/api/github/create', async (req, res) => {
    try {
        const { token, name, description, readme } = req.body;
        
        if (!token || !name || !readme) {
            return res.status(400).json({ error: "Faltan datos. Asegúrate de poner tu token." });
        }

        // Limpiar nombre del repo (sin espacios ni caracteres especiales)
        ...
        const createRes = await fetch('https://api.github.com/user/repos', {
            ...
            body: JSON.stringify({
                name: repoName,
                description: description ? description.substring(0, 300) : "Proyecto autogenerado por DevProject Generator",
                private: false,
                auto_init: false // Lo inicializamos manualmente con el README
            })
        });
        ...
        if (!createRes.ok) {
            if (createRes.status === 422) throw new Error("Ya tienes un repositorio con ese nombre exacto en tu GitHub.");
            if (createRes.status === 401) throw new Error("El Token de GitHub es inválido o expiró.");
            throw new Error(repoData.message || "Error al crear el repositorio.");
        }

        // 2. Subir el README.md como el primer commit
        const readmeBase64 = Buffer.from(readme).toString('base64');
        
        const fileRes = await fetch(`https://api.github.com/repos/${repoData.owner.login}/${repoData.name}/contents/README.md`, {
            method: 'PUT',
            headers: {
                'Authorization': `Bearer ${token}`,
                'Accept': 'application/vnd.github.v3+json',
            ...
            body: JSON.stringify({
                message: "🎉 Initial commit: Proyecto auto-generado con IA (DevProject)",
                content: readmeBase64
            })
        });

        if (!fileRes.ok) {
            // Si el readme falla, no es catastrófico, el repo ya existe.
            console.error("Error subiendo README a GitHub:", await fileRes.text());
        }

        res.json({ success: true, url: repoData.html_url });
        
    } catch (error) {
        console.error("Error Integración GitHub:", error);
